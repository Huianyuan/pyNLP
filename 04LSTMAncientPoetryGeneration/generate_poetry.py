...
def split_poetry(file="poetry_7.txt"):
    ...
    all_data_split = " ".join(all_data)  # 使用空格连接每一个字符
    with open("split7.txt", "w", encoding="utf-8")as f:
        f.write(all_data_split)
    return all_data.split("\n")  # 每一首古书一行，切分


# 词向量模型训练
def train_vec(split_file="split7.txt", org_file="poetry_7.txt"):
    vec_params_file = "vec_params.pkl"
    if os.path.exists(split_file) == False:  # 不岑仔切分文件则切分
        split_poetry()

    split_all_data = open(split_file, "r", encoding="utf-8").read().split("\n")[:1000]
    org_data = open(org_file, "r", encoding="utf-8").read().split("\n")[:1000]

    if os.path.exists(vec_params_file):  # 存在训练好的文件则直接返回参数
        return org_data, pickle.load(open(vec_params_file, "rb"))

    modle = Word2Vec(split_all_data, vector_size=107, min_count=1, workers=6)  # 模型训练
    pickle.dump((modle.syn1neg, modle.wv.key_to_index, modle.wv.index_to_key), open(vec_params_file, "wb"))  # 训练参数存储

    return org_data, (modle.syn1neg, modle.wv.key_to_index, modle.wv.index_to_key)
...
class Mymodel(nn.Module):
    ...
    def __init__(self,embedding_num,hidden_num,word_size):
        ...
        self.lstm = nn.LSTM(input_size = embedding_num,hidden_size = hidden_num,
                            batch_first = True,num_layers = 2, bidirectional = False)
