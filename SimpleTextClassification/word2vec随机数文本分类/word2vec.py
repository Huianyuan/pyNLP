...
class w2vModel(nn.Module):
    ...
    def __init__(self, embedding_num, hidden_num, class_num, max_len):
        super().__init__()
        self.linear1 = nn.Linear(embedding_num, hidden_num)  # linear层1
    ...
    def forward(self, text_embedding, labels=None):
        hidden = self.linear1.forward(text_embedding)
...
def test_file():
    ...
    test_dataset = w2vDataset(test_texts, test_labels, word_2_index, index_2_embedding, max_len)
    ...
    with open(os.path.join("../data", "test_result_word2vec.txt"), "w", encoding="utf-8") as f:
...
if __name__ == '__main__':
    ...
    # 训练集
    train_dataset = w2vDataset(train_texts, train_labels, word_2_index, index_2_embedding, max_len)
    train_dataloader = DataLoader(train_dataset, batch_size, shuffle=False)

    # 验证集
    dev_dataset = w2vDataset(dev_texts, dev_labels, word_2_index, index_2_embedding, max_len)
    dev_dataloader = DataLoader(dev_dataset, batch_size, shuffle=False)

    model = w2vModel(embedding_num, hidden_num, class_num, max_len)
